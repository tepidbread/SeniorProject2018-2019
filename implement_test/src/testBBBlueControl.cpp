...
{
double kp = 2.25;	//Proportional gain
double kd = 10;		//Derivative gain
double ki = 12.5;	//Integral gain
...
{	//Function called to calculate the current Pose of the robot
	...
	double d = 0.071; 		//Distance between robot wheels
...
int main (int argc, char **argv)
{
	...
	outputFile << "xPosition" << "yPosition\t" << "theta\t" << "Left Wheel Speed \t" <<"Right Wheel Speed\n";
...
//	while (i < 1500)
	while (ros::ok())
	{
		...
		leftEncStart = rc_encoder_eqep_read(3);
		rightEncStart = rc_encoder_eqep_read(2);	//Negitive Values Correspond With Positive Direction
		...
		outputFile << currentX << "\t" << currentY << "\t" << currentTheta << "\t" << currentLeftWheelSpeed << "\t" << currentRightWheelSpeed << endl;

		rc_motor_set(3, dutyLeft);
		rc_motor_set(2, dutyRight);
		...
		leftEncEnd = rc_encoder_eqep_read(3);
		rightEncEnd = rc_encoder_eqep_read(2);
